...
Allows admins to add new entries.
...
Add fields in `setupCreateOperation()` using the [Fields API](/docs/{{version}}/crud-fields#fields-api):
...
Set validation in `setupCreateOperation()`:
```php
$this->crud->setValidation(StoreRequest::class);
```
...
So if you want to do something to a `Product` entry _before_ it's created, you can do that:
```php
public function setupCreateOperation()
{
...
The store code is inside a trait, so you can override it, if you want:
